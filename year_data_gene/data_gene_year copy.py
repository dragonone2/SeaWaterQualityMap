...
# 생성할 행 수 정의 / 生成する行数を定義
times_values = [year * 100 + month for year in range(2018, 2024) for month in range(1, 13)]
num_rows = len(times_values)
primary_key_values = list(range(1, num_rows + 1))  # Adding primary key

# 고정 값 정의 / 固定値を定義
fixed_values = {
    "primary_key": primary_key_values,
    "times": times_values,
    "area": np.random.choice(["남해", "동해", "서해"], num_rows).tolist(),
    "coastal_name": np.random.choice(unique_coastal_names, num_rows).tolist(),
    "layer_name": ["표층"] * num_rows,
}
...
table_name = 'ocean_data_2018'
port = 3306

# 한국어: 데이터베이스 연결 URL 생성
# 日本語: データベース接続URLの生成
