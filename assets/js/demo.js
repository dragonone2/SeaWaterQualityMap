...
demo = {
    ...
    initGoogleMaps: function () {
        ...
        function initGoogleMaps() {
            ...
            // CSVファイルを取得しパースする
            fetch('./assets/js/demo.js')  // CSV 파일의 경로를 설정합니다.
                .then(response => response.text())
                .then(data => {
                    var results = Papa.parse(data, { header: true });
                    // CSV 데이터를 파싱한 결과를 사용하여 마커를 추가합니다.
                    results.data.forEach(function (row) {
                        var category = categorize(row.value);  // row.value는 5단계로 색상을 분류하는 값입니다.
                        var color = getColorForCategory(category);  // 해당 카테고리에 대응하는 색상을 가져옵니다.
                        // 마커를 생성하고 지도에 추가합니다.
                        ...
                        // 마커를 지도에 추가하기 위해 setMap()을 호출합니다.
                        marker.setMap(map);
                    });
                });

            // row.value를 기반으로 카테고리를 분류하는 함수
            function categorize(value) {
                // row.valueの値を整数に変換します。
